...
此时需要将完整方案展示给用户：先读取同目录下的 `reference.html`，**完整保留 `<style>` 块不变**，将 `<body>` 内内容替换为真实方案数据，然后调用 `save_plan_html` 工具。
...
| 6 | Agent 行为说明（≤6 条） | `.agent-actions` > `.action-item` |
| 7 | 下一步操作（≤3 条） | `.next-steps` |
...
| 审核结论 | `.audit-conclusion` | 绿色底 `#f0fdf4` |

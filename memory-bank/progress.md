...
- 状态：未完成（缺真实 Key）
- 结果：本机 `.env` 的 `LLM_API_KEY` / `EMBEDDING_API_KEY` 为空，`GET /health` 为 `ai_configured=false`。设置页已显示「未配置」。第 10 条隐私说明 UI 已具备；第 1–4、6 条需真实 Embedding/LLM，本步未打真实 DashScope。填好 Key 后再回 `1` 或 `2` 继续验收。
